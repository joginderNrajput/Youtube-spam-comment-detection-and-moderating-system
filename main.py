...
class YouTubeSpamDetectionApp:
    ...
    def create_welcome_frame(self):
        ...
        self.current_frame.pack(padx=20, pady=20, fill=tk.BOTH, expand=True)
        ...
        start_button = ttk.Button(right_frame, text="Start", command=self.create_option_selection_frame)
        start_button.pack()
    ...
    def create_option_selection_frame(self):
        ...
        submit_button = ttk.Button(right_frame, text="Submit", command=self.handle_option_selection)
        submit_button.pack()

        back_button = ttk.Button(right_frame, text="Back", command=self.go_back_to_welcome_frame)
        back_button.pack()

    def go_back_to_welcome_frame(self):
        self.create_welcome_frame()
    ...
    def handle_option_selection(self):
        ...
        if selected_option == "Scan Your Channel Video":
            ...
            self.create_help_tips_frame()
        else:
            messagebox.showerror("Invalid Option", "Please select a valid option.")

    #Function for the help and tips
    def create_help_tips_frame(self):
        if self.current_frame:
            self.current_frame.destroy()
        ...
        help_text = """
        Here are some tips to use the YouTube Spam Detection Tool:
        1. Start by entering the video ID or link of the YouTube video you want to analyze.
        2. Click on the 'Submit' button to fetch and display comments from the video.
        3. The tool will detect spam comments and display them separately.
        4. You can choose to delete or report spam comments using the respective buttons.
        5. Use the 'Back' button to navigate to the previous page.
        """

        help_label = tk.Label(self.current_frame, text=help_text, justify=tk.LEFT, font=("Arial", 10))
        help_label.pack(pady=10)

        back_button = ttk.Button(self.current_frame, text="Back", command=self.go_back_to_option_selection_frame, style="Back.TButton")
    ...
    def create_video_input_frame(self):
        ...
        submit_button = ttk.Button(right_frame, text="Submit", command=self.fetch_and_display_comments)
        submit_button.grid(row=2, column=0, padx=10, pady=10, sticky="w")

        back_button = ttk.Button(right_frame, text="Back", command=self.go_back_to_option_selection_frame)
        back_button.grid(row=3, column=0, padx=10, pady=10, sticky="w")

    def go_back_to_option_selection_frame(self):
        self.create_option_selection_frame()
    ...
    def display_spam_comments_frame(self, video_title, spam_comments, total_comments, spam_detected):
        ...
        for _, comment in spam_comments.iterrows():
            try:
                ...
                # Insert author's name with a different color and formatting
                spam_text.insert(tk.END, f"Author: ", "author")
                spam_text.insert(tk.END, f"{author}\n", "author_text")

                # Insert comment text with a different color and formatting
                spam_text.insert(tk.END, f"Comment: ", "comment")
                spam_text.insert(tk.END, f"{comment_text}\n\n", "comment_text")

                # Apply tag configurations for colors and formatting
                spam_text.tag_configure("author", foreground="blue", font=("Arial", 10, "bold"))
                spam_text.tag_configure("author_text", foreground="black", font=("Arial", 10))
                spam_text.tag_configure("comment", foreground="green", font=("Arial", 10, "italic"))
                spam_text.tag_configure("comment_text", foreground="black", font=("Arial", 10))
            except Exception as e:
                print(f"Error while processing comment: {str(e)}")
                continue

        # Display video details, total comments, and spam detected
        video_details_label = tk.Label(self.current_frame, text=f"Video Title: {video_title}", font=("Arial", 12), fg="blue")
        video_details_label.pack(pady=(15, 3))

        total_comments_label = tk.Label(self.current_frame, text=f"Total Comments Scanned: {total_comments}", font=("Arial", 10), fg="green")
        total_comments_label.pack(pady=3)

        spam_detected_label = tk.Label(self.current_frame, text=f"Spam Detected: {spam_detected}", font=("Arial", 10), fg="red")
        spam_detected_label.pack(pady=3)
        
        delete_spam_button = ttk.Button(self.current_frame, text="Delete All Spam Comments", command=self.confirm_delete_spam_comments)
        delete_spam_button.pack(pady=3)

        report_spam_button = ttk.Button(self.current_frame, text="Report All Spam Comments", command=self.confirm_report_spam_comments)
        report_spam_button.pack(pady=3)

        back_button = ttk.Button(self.current_frame, text="Back", command=self.go_back_to_create_video_input_frame)
        back_button.pack(pady=3)

    def confirm_delete_spam_comments(self):
        confirmed = messagebox.askyesno("Confirm Deletion", "Are you sure you want to delete all spam comments?")
        if confirmed:
            self.delete_all_spam_comments()

    def confirm_report_spam_comments(self):
        confirmed = messagebox.askyesno("Confirm Reporting", "Are you sure you want to report all spam comments?")
        if confirmed:
            self.report_all_spam_comments()

    def delete_all_spam_comments(self):
        # Code to delete all spam comments
        pass

    def report_all_spam_comments(self):
        # Code to report all spam comments
        pass

    def go_back_to_create_video_input_frame(self):
        self.create_video_input_frame()
